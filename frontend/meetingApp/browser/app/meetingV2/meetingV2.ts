...
import {
  loadEngagementFilters,
  platformCanSupportEngagementWithoutDegradation,
...
type VideoFilterName = 'Emojify' | 'CircularCut' | 'NoOp' | 'Engagement Detection' | 'Resize (9/16)' | 'None';
...
export class DemoMeetingApp
  ...
  loadingBodyPixDependencyTimeoutMs: number = 100000000;
  ...
  private async populateVideoFilterInputList(isPreviewWindow: boolean): Promise<void> {
    ...
    if (this.areVideoFiltersSupported()) {
      ...
      if (platformCanSupportEngagementWithoutDegradation()) {
        if (!this.loadingBodyPixDependencyPromise) {
          this.loadingBodyPixDependencyPromise = loadEngagementFilters(this.loadingBodyPixDependencyTimeoutMs);
        }
        // do not use `await` to avoid blocking page loading
        this.loadingBodyPixDependencyPromise.then(() => {
          filters.push('Engagement Detection');
  ...
  private videoFilterToProcessor(videoFilter: VideoFilterName): VideoFrameProcessor | null {
    ...
    if (videoFilter === 'Engagement Detection') {
